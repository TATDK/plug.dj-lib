var Service = Class.extend({
    ...
    socket, Socket = Class.extend({
        ...
        close: function (a) {
            a && a.wasClean ? alert("This session has now ended. Goodbye.") : (console.info("closing",
            a), a = this.backoff, 5 >= a && (a += 1), socketInit(a))
        },
        ...
        message: function (a) {
            ...
            if (a.type && "rpc" === a.type) {
                ...
                if (b) {
                    var c = this.rpcs[b];
                    if (c) if (delete this.rpcs[b], 999 === a.status) this.socketLog(a),
                    c.onFault(a.result);
                    else c.onResult({
                        status: a.status,
                        data: a.result
...
var SocketMessageListener = Class.extend({
    ...
    pdjUpdate: function () {
        ...
        Dialog.alert(Lang.alerts.updateMessage, $.proxy(Utils.forceRefresh,
        Utils), Lang.alerts.update, !0)
    },
    ...
    SoundCloudPlaylistModel = Class.extend({
        ...
        importSelected: function () {
            this.createPlaylist(this.data[this.selectedIndex].title,
            this.data[this.selectedIndex].tracks, 1);
    ...
    YouTubePlaylistModel = Class.extend({
        ...
        load: function (a) {
            this.data = [];
            Models.search.facade.ytPlaylist(a,
            $.proxy(this.onComplete, this))
        },
        onComplete: function (a) {
            this.data = a;
            this.updateCallback(a)
        ...
        importAlert: function () {
            Dialog.alert(Lang.alerts.youtubeImporting,
    ...
    SearchModel = Class.extend({
        ...
        load: function (a, b, c) {
            ...
            this.lastFormats = b;
            this.initCounts(c);
            "1" == b ? (this.ytRequired = !0,
            this.facade.ytSearch(a, c, 30, $.proxy(this.onYTResult, this))) : "2" == b && (this.scRequired = !0, this.facade.scSearch(a, c, 30, $.proxy(this.onSCResult, this)))
        },
        ...
        loadSCFavorites: function (a) {
            ...
            this.facade.scFavorites(a, 30, $.proxy(this.onSCResult,
            this))
        },
        ...
        setPagination: function (a) {
            a ? $("#media-panel-pagination-value").text(this.start + " - " + this.end) : $("#media-panel-pagination-value").text(this.start +
                " - ...");
            0 == this.page ? ($("#media-panel-pagination-left-button").css("opacity", 0.2), $("#media-panel-pagination-left-button").css("cursor", "default")) : ($("#media-panel-pagination-left-button").css("opacity", 1), $("#media-panel-pagination-left-button").css("cursor", "pointer"));
            "" == this.end || this.relatedSearch ? ($("#media-panel-pagination-right-button").css("opacity", 0.2), $("#media-panel-pagination-right-button").css("cursor", "default")) : ($("#media-panel-pagination-right-button").css("opacity", 1), $("#media-panel-pagination-right-button").css("cursor",
                "pointer"))
        },
        onNextPage: function () {
            !this.relatedSearch && "" != this.end && (!this.scFavoritesLookup && !this.scTracksLookup ? this.load(this.lastQuery, this.lastFormats, this.page + 1) : this.scFavoritesLookup ? this.loadSCFavorites(this.page + 1) : this.scTracksLookup && this.loadSCTracks(this.page + 1))
        },
        onPrevPage: function () {
        ...
        appendUnknown: function (a) {
            ...
            c = 0; c < b; ++c) this.hash[a[c].cid] || this.data.push(a[c])
        }
    }),
    ...
    UserModel = Class.extend({
        ...
        changeLanguage: function (a) {
            this.data.language != a && ((new UserLanguageUpdateService(a)).successCallback = $.proxy(this.onLanguageUpdateSuccess, this))
        },
    ...
    RoomModel = EventDispatcher.extend({
        ...
        voteUpdate: function (a) {
            this.userHash[a.id] ? (this.userHash[a.id].vote = a.vote, this.data.votes[a.id] = a.vote, this.earnedPoints[a.id] = 1, this.earnedPoints[this.data.currentDJ] && this.earnedPoints[this.data.currentDJ].v && (1 == a.vote && 1 != this.earnedPoints[this.data.currentDJ].v[a.id] ? (++this.earnedPoints[this.data.currentDJ].dj, this.earnedPoints[this.data.currentDJ].v[a.id] = 1) : -1 == a.vote && 1 == this.earnedPoints[this.data.currentDJ].v[a.id] && (--this.earnedPoints[this.data.currentDJ].dj, this.earnedPoints[this.data.currentDJ].v[a.id] = -1)), a.id != Models.user.data.id && this.dispatchEvent("voteUpdate", {
                noRefresh: !1,
        ...
        curateUpdate: function (a) {
            this.userHash[a.id] ? (this.userHash[a.id].curated = a.curated, this.data.curates[a.id] = a.curated, this.earnedPoints[this.data.currentDJ] && ++this.earnedPoints[this.data.currentDJ].c, this.updateRoomScore(), this.updateEarnedUserMeta()) : console.log("CurateUpdateError :: user " + a.id + " does not exist in room")
        },
        roomPropsUpdate: function (a) {
        ...
        djUpdate: function (a) {
            ...
            this.buildDJHash();
            b && (this.checkCurrentDJ(), this.checkStaffAvatars(), RoomUser.setDJs(this.getDJs()), RoomUser.audience.refresh(),
            5 > this.data.djs.length ? UserListOverlay.hide() : UserListOverlay.updateWaitList(), API.delayDispatch(API.DJ_UPDATE, this.getDJs()))
        },
    ...
    LobbyOverlayView = Class.extend({
        ...
        posLobbyLabels: function () {
            $.browser.mozilla && ($(".room-media").css("left", 81), $(".room-djs").css("bottom", 1), $(".room-djs-label").css("top", 1), $(".room-friends").css("bottom", 1), $(".room-friends-label").css("top", 1));
            Main.isMac && ($.browser.mozilla ? ($(".room-djs-label").css("top", 2), $(".room-friends-label").css("top", 2)) : $(".room-population-value").css("top",
            11))
        },
        onScroll: function () {
    ...
    AvatarOverlayView = Class.extend({
        ...
        draw: function () {
            ...
            for (var a = this.avatarSets[this.selectedSet],
            b = a.length, c = 10, d = 0; d < b; ++d) {
                ...
                5 < f ? 6 != f ? (j.append(this.drawRow(e.slice(0, 5), h, a[d].required - g)), j.append(this.drawRow(e.slice(5, f), h, a[d].required - g).css("top", 150))) : (j.append(this.drawRow(e.slice(0, 4), h, a[d].required - g)), j.append(this.drawRow(e.slice(4, f), h, a[d].required - g).css("top", 150))) : j.append(this.drawRow(e, h, a[d].required - g));
                e = 5 < f ? 400 : 250;
                j.height(e);
                c += e;
...
var RoomView = Class.extend({
    ...
    RoomUserView = Class.extend({
        init: function () {
            $("#button-vote-negative").css("cursor", "default").click($.proxy(this.onButtonVoteNegativeClick, this));
...
function avatarTick(a) {
    a - tickStart > animSpeed && (tickStart = a, RoomUser.animateAvatars());
...
var MediaOverlayView = Class.extend({
    ...
    onPlaylistUpdate: function (a) {
        ...
        $("#media-menu-playlist-container").empty().append(b);
